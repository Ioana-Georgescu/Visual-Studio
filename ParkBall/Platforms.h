...
{
	// color codes
	#define TOUCHED 0 // player has landed on it
	#define NORMAL 1 // normal
	#define PLUSFUEL 2 // gain fuel
	#define MINUSFUEL 3 // lose fuel
	#define GOGO 4 // stuck on max speed
	#define DEATH 5 // game end
	#define MINUSLIFE 6 // lose a life
	#define PLUSLIFE 7 // add a life
	#define GHOST 8 // pass through walls

	#define LANE_WIDTH 2
	#define PLATFORM_THICKNESS 0.5
	#define PLATFORM_BOTTOM -0.5
	#define FIELD_LENGHT 100
	#define WALLTHICKNESS 0.25 // thinner than player sphere
	#define SPRING_DISTANCE 8 // min dist between player and wall for spring wall to rise
	#define WALL_RISE_SPEED 2.5
	#define DIST_FOR_JUMP 3 // player must be able to start jump on platform and have room to land
	...
	struct PlarformLog
	{
		PlarformLog() : x(0), startZ(0), endZ(1), colorCode(0), active(false) {}
		PlarformLog(float x, float startZ, float endZ, int colorCode, bool active)
			: x(x), startZ(startZ), endZ(endZ), colorCode(colorCode), active(active) {}
		float x;
		float startZ;
		float endZ;
		int colorCode;
	...
	struct WallLog
	{
		WallLog() : x(0), startZ(0), hight(0), spring(false), finalHight(0), texture(0) {}
		WallLog(float x, float startZ, float hight, bool spring, float finalHight, int texture)
			: x(x), startZ(startZ), hight(hight) , spring(spring), finalHight(finalHight), texture(texture) {}
		float x;
		float startZ;
		float hight;
		bool spring;
	...
	public:
		...
		void InitNewPlatform(float lane, int colorCode, float lastRowEnd)
		{
			...
			logList.push_back(PlarformLog(lane, startZ, endZ, colorCode, true));
		}
		...
		void InitNewWall(float lane, float platformStartZ, float platformEndZ, float hight, int texurePoolSize)
		{
			float startZ;
			startZ = -((float)(rand() % (int)(platformStartZ - platformEndZ - DIST_FOR_JUMP)) - platformStartZ + DIST_FOR_JUMP);

			float springChance = rand() % 100;
			int texture = rand() % texurePoolSize;
			if (springChance < springLimit)
			{
				walls.push_back(WallLog(lane, startZ, 0, true, hight, texture));
			}
			else
			{
				walls.push_back(WallLog(lane, startZ, hight, false, hight, texture));
			}
		}
		...
		void AddPlatforms(int lives, int texurePoolSize)
		{
				...
				for (int i = 0; i < auxLanes.size(); i++)
				{
					if (laneCode % 10 == 1) // platform on this lane
					{
						...
						if (wallChance < wallLimit)
						{
							...
							InitNewWall(auxLanes[i], logList[logList.size() - 1].startZ,
								logList[logList.size() - 1].endZ, AnyHight(), texurePoolSize);
						}
						else
						{
				...
				if (laneCode % 10 == 1)
				{
					if (safeCode % 10 == 1)
					{
						...
						if (wallChance < wallLimit)
						{
							...
							InitNewWall(mainLanes[i], logList[logList.size() - 1].startZ,
								logList[logList.size() - 1].endZ, SafeHight(), texurePoolSize );
						}
						else
						{
					...
					if ((logList[logList.size() - 1].colorCode == DEATH)
